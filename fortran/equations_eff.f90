...
!       end if
    ...
    real(dl) wprime, w_eff, Hv3_over_k
    k=EV%k_buf
    k2=EV%k2_buf
    ...
    if (.not. is_cosmological_constant) then
        ...
        Hv3_over_k =  3*adotoa* qde / k
        ayprime(EV%w_ix) = -3 * adotoa * (cs2_lam - w_eff) * (clxde + Hv3_over_k) &
              -  k * qde - (1 + w_eff) * k * z - adotoa*wprime*Hv3_over_k
        ayprime(EV%w_ix + 1) = -adotoa * (1 - 3 * cs2_lam - wprime) * qde + k * cs2_lam * clxde
